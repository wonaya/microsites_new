angular.module('AgaveToGo').controller('AppsResourceRunController', function($scope, $stateParams, $uibModal, $uibModalStack, $localStorage, $rootScope, $translate, AppsController, SystemsController, JobsController, NotificationsController, FilesController, MessageService, moment) {
    ...
    $scope.formSchema = function(app) {
      ...
      schema.properties.archive = {
        ...
        description: 'If checked, the job work directory will be archived upon completion.',
        type: 'boolean',
        required: true,
      };
        ...
        AppsController.getAppDetails($rootScope.settings.appId)
          .then(
            function(response){
              ...
              $scope.form = {model: {
                  archive: true,
                  name: $scope.app.name + '-' + moment().format('X'),
                  }};
              ...
              if ($scope.form.schema.properties.parameters && Object.keys($scope.form.schema.properties.parameters.properties).length > 0) {
                ...
                angular.forEach($scope.form.schema.properties.parameters.properties, function(param, key){
                    if (param.type == 'number') {
                        parameters[0].items.push(
                        {
                            "input": key,
                            "type": "template",
                            "template": '<div class="form-group has-success has-feedback"> <label for="input">{{form.title}}</label> <input type="number" class="form-control" id="param-{{form.input}}" ng-model="form.model.parameters[form.input]"> <span class="help-block">{{form.description}}</span> </div>',
                            "title": param.title,
                        ...
                        parameters[0].items.push(
                        {
                            "input": key,
                            "type": "template",
                            "template": '<div class="form-group has-success has-feedback"> <label for="input">{{form.title}}</label> <input type="text" class="form-control" id="param-{{form.input}}" ng-model="form.model.parameters[form.input]"> <span class="help-block">{{form.description}}</span> </div>',
                            "title": param.title,
                            "description": param.description,
                            "model": $scope.form.model,
                            selectFile: function(key){
              ...
              $scope.form.form.push({
                type: 'fieldset',
                title: 'Job details',
                items: ['requestedTime','name', 'archive', 'archivePath']
              });
