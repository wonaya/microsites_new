angular.module('AgaveToGo').controller('FileExplorerController', function ($rootScope, $scope, $http, $timeout, $filter, $localStorage, $location, $state, $stateParams, $uibModal, $translate, SystemsController, FilesController, MessageService, ProfilesController) {
    ...
    // if no system is set, force the system specified in the site config
    // as the default.
    if (!$stateParams.systemId || 0 === $stateParams.systemId.length) {
        $stateParams.systemId = $rootScope.$settings.storageSystemId;
    }
